		...
		$('#btnUpdate').on('click',function(){
			...
			$.ajax({ 
			    ...
			    data: JSON.stringify( { id, name,password, role } ),
					//필드명과 값의 이름이 같다면 알아서 처리 = json축약형
			    contentType: 'application/json' //요청파라미터 json (MIME 타입)
					//data: $('#form1').serialize()
					//serialize : 제이쿼리 함수 폼태그가 있으면 알아서 문자열로 만들어줌(질의문자열 즉, 쿼리스트링) 'id=ddd&name=aaa&password=aaa&role=Admin'
			}).done( function(data) { 
			        userList();
